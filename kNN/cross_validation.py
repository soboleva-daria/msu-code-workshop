...
def kfold(n, n_folds=3, shuffle=True):
...
def knn_cross_val_score(X, y, k_list=[3], score='accuracy', cv=None,
    ...
    cv = _check_cv(cv, X)
    klist = _check_klist(k_list)
    score = _check_scoring(score)
    scores = {}
...
def _fit_and_score(estimator, X, y, score,
